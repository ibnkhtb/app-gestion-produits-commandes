...
public class CommandeResource {
    ...
    public ResponseEntity<CommandeDTO> createCommande(@RequestBody CommandeDTO commandeDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/commandes/" + result.getIdCommande()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getIdCommande().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /commandes/:idCommande} : Updates an existing commande.
     *
     * @param idCommande the id of the commandeDTO to save.
    ...
    public ResponseEntity<CommandeDTO> updateCommande(
        ...
        if (!Objects.equals(idCommande, commandeDTO.getIdCommande())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, commandeDTO.getIdCommande().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /commandes/:idCommande} : Partial updates given fields of an existing commande, field will ignore if it is null
     *
     * @param idCommande the id of the commandeDTO to save.
     * @param commandeDTO the commandeDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated commandeDTO,
    ...
    public ResponseEntity<CommandeDTO> partialUpdateCommande(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, commandeDTO.getIdCommande().toString())
        );
    }
